...
alter table agencies add constraint phone_number check ( phone_number like '+996_________');
...
select owners.first_name,h.house_type from owners join rent_info ri on owners.id = ri.owner_id join Houses H on owners.id = H.owner_id
order by owners.first_name like '________';
...
select address_id,street,house_type from Houses join addresses a on a.id = Houses.address_id where address_id in(5,6,7,8,9,10);
-- 4. Бардык уйлорду, уйдун ээсинин атын, клиенттин атын, агенттин атын чыгар.
select o.first_name,c.first_name,a.name from Houses join rent_info ri on Houses.id = ri.house_id join customers c on c.id = ri.customer_id join owners o on o.id = Houses.owner_id join agencies a on a.id = ri.agency_id;
-- 5. Клиенттердин 10-катарынан баштап 1999-жылдан кийин туулган 15 клиентти чыгар.
select first_name from customers group by first_name order by first_name limit 15 offset 10;
-- 6. Рейтинги боюнча уйлорду сорттоп, уйлордун тайптарын, рейтингин жана уйдун ээлерин чыгар. (asc and desc)
select H.house_type,H.rating,o.first_name from rent_info join Houses H on H.id = rent_info.house_id join rent_info r on r.house_id=r.id join owners o on r.owner_id= o.id  order by rating;
select H.house_type,H.rating,o.first_name from rent_info join Houses H on H.id = rent_info.house_id join rent_info r on r.house_id=r.id join owners o on r.owner_id= o.id  order by rating desc ;
